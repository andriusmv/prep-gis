    ...
    <>
      <div className={styles.sidebar}>
        <aside className={styles.sidebar__sidebar}>
          ...
          <p className={styles.sidebar}>ArcGIS Desktop Professional 19-001</p>
          <p className={styles.sidebar}><span>EADP19-001 exam</span></p>
          <br />
        ...
        <ul className={styles.mainvideo}>
          {video.map((video: {
                ...
                video_url: string;}) => (
                <li key={video.id}>
                <Link href={`/exam/EADP19-001/video#${video.id}`} className={styles.container}>{video.title}</Link>
                </li>
                ))}
        </ul>
        </aside>

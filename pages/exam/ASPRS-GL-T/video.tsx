    ...
    <>
      <div className={styles.sidebar}>
        <aside className={styles.sidebar__sidebar}>
          ...
          <p className={styles.sidebar}>ASPRS Certified GIS/LIS Technologist</p>
          <p className={styles.sidebar}><span>ASPRS-GL-T exam</span></p>
          <br />
        ...
        <ul className={styles.mainvideo}>
          {video.map((video: {
                ...
                video_url: string;}) => (
                <li key={video.id}>
                <Link href={`/exam/ASPRS-GL-T/video#${video.id}`} className={styles.container}>{video.title}</Link>
                </li>
                ))}
        </ul>
        </aside>
...
export const getStaticProps: GetStaticProps = async (context) => {
  const { data: video } = await supabase.from("video").select().or('tag.cs.{ASPRS-GL-T}');

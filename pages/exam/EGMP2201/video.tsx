    ...
    <>
      <div className={styles.sidebar}>
        <aside className={styles.sidebar__sidebar}>
          ...
          <p className={styles.sidebar}>Enterprise Geodata Management Professional 2201</p>
          <p className={styles.sidebar}><span>EGMP2201 exam</span></p>
          <br />
        ...
        <ul className={styles.mainvideo}>
          {video.map((video: {
                ...
                video_url: string;}) => (
                <li key={video.id}>
                <Link href={`/exam/EGMP2201/video#${video.id}`} className={styles.container}>{video.title}</Link>
                </li>
                ))}
        </ul>
        </aside>

    ...
    <>
      <div className={styles.sidebar}>
        <aside className={styles.sidebar__sidebar}>
          ...
          <p className={styles.sidebar}>AlaskaX's Geographic Information Systems (GIS) Essentials Professional Certificate</p>
          <p className={styles.sidebar}><span>AlaskaX-0 exam</span></p>
          <br />
        ...
        <ul className={styles.mainvideo}>
          {video.map((video: {
                ...
                video_url: string;}) => (
                <li key={video.id}>
                <Link href={`/exam/AlaskaX-02101/video#${video.id}`} className={styles.container}>{video.title}</Link>
                </li>
                ))}
        </ul>
        </aside>
